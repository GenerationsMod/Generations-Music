...
//              Set the items tag in inventory, by getting the tag and setting the "Items" tag to the resulting ListTag from Inventories.toTag() using the stacks from the current inventory
        ...
        root.add(new WButton(Component.literal("⏭")).setOnClick(() -> changeSelectedSlot(Math.min((rows * 9) - 1, DiscHolderHelper.getSelectedSlot(playerInv.getItem(DiscHolderHelper.getSlotFromUUID(playerInv, uuid))) + 1))), 6, 2 + rows - 1);
        ...
        var slider = new WLabeledSlider(0, 100, Axis.HORIZONTAL, Component.translatable("text.musicexpansion.volume"));

        slider.setLabelUpdater(new WLabeledSlider.LabelUpdater() {
            @Override
            public Component updateLabel(int i) {
        ...
        root.add(slider, 0, 3 + rows - 1, 6, 1);
        root.add(new WPlayerInvPanel(playerInventory, true), 0, 4 + rows - 1);

        var slot = DiscHolderHelper.getSelectedSlot(holder);
        selected.setLocation((slot % 9) * 18, (slot/9) * 18);
        setRootPanel(root);
        root.validate(this);
    }
